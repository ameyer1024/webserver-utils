...
pub fn process_markdown(text: &str, base_url: Option<&str>) -> (String, Vec<String>) {
    ...
    fn ensure_signature<
        F: for<'a> Fn(pulldown_cmark::BrokenLink<'a>) -> Option<(CowStr<'a>, CowStr<'a>)>,
    >(
        f: F,
    ) -> F {
        f
    ...
    let callback = ensure_signature(|l: pulldown_cmark::BrokenLink| {
        ...
        let wiki_link = matches!(
            (
                text[..l.span.start].chars().last(),
                text[l.span.end..].chars().next()
            ),
            (Some('['), Some(']'))
        );
    ...
    let parser =
        pulldown_cmark::Parser::new_with_broken_link_callback(text, options, Some(callback));
