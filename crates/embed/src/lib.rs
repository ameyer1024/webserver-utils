...
impl Info {
    ...
    pub fn blank_embed(url: &str) -> Info {
        ...
        if let Some(default) = parsed_url
            .as_ref()
    ...
    pub fn direct_image(url: &url::Url, content_type: Option<&str>) -> Self {
        ...
        this.opengraph
            ...
            .push(OpenGraphObj {
                ...
                properties: []
                    .into_iter()
                    .chain(if let Some(t) = content_type {
                        Some(("type".into(), t.into()))
                    } else {
                        None
                    })
                    .collect(),
    ...
    pub fn best_icon(&self, target: u32, target_range: std::ops::Range<u32>) -> Option<&Icon> {
        ...
        let res = self
            ...
            .max_by_key(|(i, icon)| {
                ...
                let index = if matches!(icon.kind, IconKind::DefaultFavicon) {
                    -1
                } else {
                    *i as i32
                };
                let diff = if size < target {
                    -4096
                } else {
                    target as i32 - size as i32
                };
        ...
        match name {
            ...
            _ => {
                if let Some((scope, prop)) = name.split_once(':') {
                    ...
                    let last = if let Some(l) = list.last_mut() {
                        l
                    } else {
                        list.push(OpenGraphObj {
                            url: None,
                            properties: Default::default(),
                        });
...
pub fn parse_document(bytes: &[u8], content_type: Option<&str>, url: &url::Url) -> Info {
    ...
    for tag in document.select(&meta_tags) {
        if let Some(_charset) = tag.attr("charset") {
            ...
            tag.attr("name").or_else(|| tag.attr("property")),
            tag.attr("content"),
        ) {
    ...
    for tag in document.select(&link_tags) {
        ...
        if let Some(rel) = tag.attr("rel") {
            ...
            let resolved_href: Option<String> = tag.attr("href").map(|u| {
                base_url
                    .join(u)
                    .map(Into::into)
    ...
    if description.is_none()
        && url
            .domain()
            .map(|s| s.ends_with("wikipedia.org") || s.ends_with("wikipedia.org."))
            .unwrap_or(false)
    {
        let wikipedia_shortdesc = Selector::parse(
            "div.shortdescription, #shared-image-desc #fileinfotpl_desc + td .description.en",
        )
        .unwrap();
        let wikipedia_first_para =
            Selector::parse("#mw-content-text > div > p:first-of-type").unwrap();
...
fn normalize_whitespace_text<'a>(
